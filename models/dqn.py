...
import numpy as np
# Import deque (double-ended queue) for the replay buffer
from collections import deque
# Import random for exploration (epsilon-greedy policy)
import random
...
class DQN(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, output_size):
        ...
        # Create second fully connected layer: hidden -> hidden
        self.fc2 = nn.Linear(hidden_size, hidden_size)
        # Create output layer: hidden -> action values (Q-values)
    ...
    def forward(self, x):
        ...
        # ReLU: max(0,x) - keeps positive values unchanged, converts negatives to 0
        x = F.relu(self.fc1(x))
        # Apply second linear layer followed by ReLU activation
        x = F.relu(self.fc2(x))
        # Apply final linear layer (no activation - raw Q-values)
        return self.fc3(x)
...
class ReplayBuffer:
    ...
    # Returns: batch of transitions as tensors
...
# DQN Agent that makes decisions and learns from experience
class DQNAgent:
    # Constructor method
    # Input parameters:
    ...
    def __init__(self, state_size, action_size, hidden_size=128, lr=1e-4):
        ...
        self.epsilon_decay = 0.9995  # Decay factor for exploration rate
        self.epsilon_decay_start = 1000  # Start decaying epsilon after this many steps
    ...
    def train_step(self):
        ...
        # Compute target Q-values (Bellman equation): r + γ * max_a' Q(s_{t+1}, a')
        with torch.no_grad():  # Don't track gradients for target computation
            # Get maximum Q-value for each next state using target network
